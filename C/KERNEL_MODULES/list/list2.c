...
static void mylist_del_head(void) {
  struct mylist_entry *e = list_first_entry(&mylist, struct mylist_entry, list); // listの最初の値を取得
  int n = e->n; // Message出力用に値を取得
  list_del(&e->list); // 対象エントリをリストから削除
  kfree(e); // 確保していたエントリ用のメモリを開放
